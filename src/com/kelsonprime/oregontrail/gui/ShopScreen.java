...
public class ShopScreen extends JPanel{
	...
	public ShopScreen(Shop shop){
		...
		panel.setForeground(UIManager.getColor("OptionPane.warningDialog.titlePane.foreground"));
		add(panel, BorderLayout.NORTH);
		
		JLabel lblWelcomeToThe = new JLabel("Welcome to the " + shop.getName()+ " Shop");
		lblWelcomeToThe.setForeground(Color.WHITE);
		panel.add(lblWelcomeToThe);
		
		JPanel panel_1 = new JPanel();
		panel_1.setBackground(UIManager.getColor("OptionPane.warningDialog.titlePane.shadow"));
		panel_1.setForeground(UIManager.getColor("OptionPane.warningDialog.titlePane.shadow"));
		...
		JLabel lblAxleprice = new JLabel("$"+Integer.toString(shop.getAxlePrice()));
		lblAxleprice.setBounds(506, 45, 61, 15);
		panel_1.add(lblAxleprice);
		
		JLabel lblWheelprice = new JLabel("$"+Integer.toString(shop.getWheelPrice()));
		lblWheelprice.setBounds(505, 76, 61, 15);
		panel_1.add(lblWheelprice);
		
		JLabel lblOxenprice = new JLabel("$"+Integer.toString(shop.getOxenPrice()));
		lblOxenprice.setBounds(506, 14, 70, 15);
		panel_1.add(lblOxenprice);
		
		JLabel lblTongueprice = new JLabel("$"+Integer.toString(shop.getTonguePrice()));
		lblTongueprice.setBounds(506, 107, 70, 15);
		panel_1.add(lblTongueprice);
		
		JLabel lblClothesprice = new JLabel("$"+Integer.toString(shop.getClothesPrice()));
		lblClothesprice.setBounds(506, 138, 70, 15);
		panel_1.add(lblClothesprice);
		
		JLabel lblBulletprice = new JLabel("$"+Integer.toString(shop.getBulletPrice()));
		lblBulletprice.setBounds(506, 170, 70, 15);
		panel_1.add(lblBulletprice);
		
		JLabel lblFoodprice = new JLabel("$"+Integer.toString(shop.getFoodPrice()));
